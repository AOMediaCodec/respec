...
(function () {
  ...
  const modules = [
    ...
    Promise.resolve().then(function () { return l10n$n; }),
  ...
  function run$L(config) {
    config.l10n = l10n$m[lang$2] || l10n$m.en;
  }

  var l10n$n = /*#__PURE__*/Object.freeze({
    __proto__: null,
    name: name$U,
    l10n: l10n$m,
    lang: lang$2,
  ...
  function joinAnd(array = [], mapper = item => item, lang = lang$2) {
    ...
    switch (items.length) {
      ...
      case 2: // x and y
        return items.join(l10n$l.x_and_y);
      default: {
        // x, y, and z
        const str = items.join(", ");
        const lastComma = str.lastIndexOf(",");
        const and = l10n$l.x_y_and_z;
  ...
  function htmlJoinAnd(array, mapper = item => item) {
    ...
    switch (items.length) {
      ...
      case 1: // "x"
        return items[0];
      case 2: // x and y
        return html`${items[0]}${l10n$l.x_and_y}${items[1]}`;
      default: {
        const joined = htmlJoinComma(items.slice(0, -1));
        return html`${joined}${l10n$l.x_y_and_z}${items[items.length - 1]}`;
      }
    }
  }
  ...
  const name$S = "core/respec-global";
  ...
  const name$R = "core/post-process";
  ...
  const name$Q = "core/pre-process";

  async function run$H(config) {
  ...
  async function runAll(plugs) {
    ...
    run$K(respecConfig);
    run$J(respecConfig);
    performance.mark(`${name$P}-start`);
    await run$H(respecConfig);
    ...
    performance.mark(`${name$P}-end`);
    performance.measure(name$P, `${name$P}-start`, `${name$P}-end`);
  }
...
>
  ...
  const name$L = "core/location-hash";
...
`;
  ...
  const name$H = "core/data-include";
  ...
  async function run$A() {
    ...
    const promisesToInclude = Array.from(includables).map(async el => {
      ...
      try {
        ...
        showError(msg, name$H, { elements: [el] });
      }
    });
  ...
  const l10n$k = getIntlData(localizationStrings$m);
  ...
  function run$z(conf) {
    ...
    if (h1Elem.isConnected && h1Elem.textContent.trim() === "") {
      ...
      showError(msg, name$G, { title, elements: [h1Elem] });
    }
  ...
  function setDocumentTitle(conf, h1Elem) {
    ...
    if (!h1Elem.isConnected) {
      h1Elem.textContent = document.title || `${l10n$k.default_title}`;
    }
  ...
  const name$F = "core/templates/show-link";
  ...
  const name$E = "core/templates/show-logo";
  ...
  function showLogo(logo) {
    ...
    if (!logo.alt) {
      ...
      showWarning(msg, name$E, { elements: [a] });
    }
  ...
  const name$D = "core/templates/show-people";
...
>
  ...
  function showPeople(persons = []) {
    const l10n = localizationStrings$l[lang$1];
    ...
    ></dd>`;
      ...
      if (p.retiredDate) {
        ...
        if (!isValidDate) {
          ...
          showError(msg, name$D, { title, elements: [timeElem] });
        }
  ...
  var headersTmpl = conf => {
    ...
    <dl>
      <dt>${l10n$j.this_version}</dt>
      <dd>
        <a class="u-url" href="${conf.thisVersion}">${conf.thisVersion}</a>
      </dd>
      <dt>${l10n$j.issue_tracker}</dt>
      <dd>
        <a class="u-url" href="${conf.issueTracker}">${conf.issueTracker}</a>
      </dd>
      <dt>${conf.multipleEditors ? l10n$j.editors : l10n$j.editor}</dt>
      ${showPeople(conf.editors)}
      ${Array.isArray(conf.formerEditors) && conf.formerEditors.length > 0
        ? html`
            <dt>
              ${conf.multipleFormerEditors
                ? l10n$j.former_editors
                : l10n$j.former_editor}
            </dt>
      ...
      ${conf.authors
        ? html`
            <dt>${conf.multipleAuthors ? l10n$j.authors : l10n$j.author}</dt>
